  ...
  const download = async () => {
    if (torrentDb.status === 'waiting') {
      ...
      await axios.post(`${config.qbt_server}/api/v2/torrents/add`, formData, {
        headers: { Cookie: authRes['set-cookie'][0], ...formData.getHeaders() }
      })
      ...
      return new Promise((resolve) => {
        const interval = setInterval(async () => {
          try {
            ...
            console.log(torrent.data.total_downloaded, torrent.data.total_size)
            // Upload file
    ...
    return new Promise((resolve) => {
      const interval = setInterval(async () => {
        ...
        if (rcloneJob.finished) {
          ...
          await axios.post(`${config.jellyfin_server}/Library/Refresh`, null, {
            params: {
